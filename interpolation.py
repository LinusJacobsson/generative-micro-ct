...
class TumorDataSet(Dataset):
      ...
      def __getitem__(self, idx):
          ...
          next_index = min(idx+1, len(self.image_files))
          ...
          return input_images, current_image
...
num_images = 100  # for example, to take the first 100 images
...
def show_images(images, title):
    ...
    for i, img in enumerate(images):
        axs[i].imshow(img.permute(1, 2, 0), cmap='gray')  # permute to rearrange channels for display
        axs[i].axis('off')
    plt.suptitle(title)
    plt.show()


# Iterate through the DataLoader and visualize images
for batch_idx, (input_images, target_image) in enumerate(dataloader):
    # Split concatenated images into separate images
    prev_image, current_image, next_image = torch.split(input_images.squeeze(), 1, dim=0)

    # Display images
    show_images([prev_image, current_image, next_image], title=f"Batch {batch_idx}: Previous, Current, Next")

    # Optionally break the loop to display only the first few batches
    if batch_idx == 1:  # Change this number to display more batches
        break
